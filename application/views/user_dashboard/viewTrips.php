...
<main class="page-content section-90 section-md-111 text-md-left">
	<section class="offset-top-60 offset-md-top-111">
    	<div class="shell">
        <h2 class="text-bold">Travelling History</h2>
              			...
              			<tr>
							...
							<td> 
								...
								<form action="<?php echo site_url('index.php/user_dashboard/track_bus') ?>" method="post" target="_blank" class="form-horizontal">
									...
									<div class="form-group">
										<div class="col-sm-4">
											<input type="submit" class="btn btn-xs btn btn-ellipse-type-2 btn-ripe-lemon" value="Track Bus" onClick="return is_datevalid(<?php echo $p['trav_date'];?>);">
										</div>
									</div>
								</form>
								<?php } 
...
<script>
	...
	function error1(){
		alert("Ticket refunded. So feature is not valid.");
		return false;
	}
	function is_datevalid(date){
		var today = new Date();
		...
		if(today !== date){
			alert("Feature will be live on travelling date only.");
			return false;
		}
	}
</script>