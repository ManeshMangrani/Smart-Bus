        ...
        <h2 class="text-bold">Travelling Info</h2>
        	...
        	<div class="range range-xs-center range-md-left offset-top-30 offset-md-top-65">
            	<div class="cell-xs-10 cell-lg-9">
            		...
            		<table data-responsive="true" class="table table-custom table-primary table-fixed">
              			<b><tr>
                      <?php if($this->session->userdata('old_tid')){ ?>
                      <th>Info Status</th>
                      <?php } ?>
                			<th>Source</th>
						          <th>Destination</th>
                  ...
                  <?php if($this->session->userdata('old_tid')){ ?>
                    <tr>
                      ...
                      <th>Old</th>
                      <?php } ?>
                      <td> <?php echo $this->session->userdata('old_source'); ?></td>
                      <td> <?php echo $this->session->userdata('old_destination'); ?></td>
                      <td> <?php echo $this->session->userdata('old_busdate').' - '.$this->session->userdata('old_departure'); ?> </td>
                      <td> <?php echo $this->session->userdata('old_arrival').' - '; 
                       if($this->session->userdata('old_day') == 1){ echo "Next Day";}
                       else{echo "Same Day";}; ?>
                      </td>
                    </tr>
                    <?php } ?>
        	...
        	<div class="range range-xs-center range-md-left offset-top-30 offset-md-top-65">
            	<div class="cell-xs-10 cell-lg-9">
            		...
            		<table data-responsive="true" class="table table-custom table-primary table-fixed">
                      ...
                      <?php } ?>
                			<td> <?php echo $this->session->userdata('km'); ?></td>
							        ...
							        <td> 
                      <?php if($this->session->userdata('old_tid')){ ?>
                          <form action="<?php echo site_url('index.php/ticket/book_exchange') ?>" method="post" class="form-horizontal">
                      ...
                      <?php } else{ ?>
								      <form action="<?php echo site_url('index.php/ticket/book') ?>" method="post" class="form-horizontal">
						 	  		    <div class="form-group">
										    <div class="col-sm-4">
											    <input type="submit" class="btn btn-xs btn btn-ellipse-type-2 btn-ripe-lemon" value="Book">
										    </div>
						 	  		    </div>
						 		      </form> <?php } ?>
							        </td>
                    ...
                    <tr>
                      ...
                      <th>Old</th>
                      <?php } ?>
                      <td> <?php echo $this->session->userdata('old_km'); ?></td>
                      <td> <?php if($this->session->userdata('logged_in')){echo $this->session->userdata('old_offer_price') * $this->session->userdata('old_num-seat').'/'.$this->session->userdata('old_offer_value');}
                      else echo $this->session->userdata('old_offer_price'); ?></td>
                      <td> <?php echo $this->session->userdata('old_seatno'); ?> </td>
                      <td></td>
                    </tr>
                    <?php } ?>
            		</table>
