...
class Ticket_model extends CI_Model {
    ...
    public function postbooking_exchange(){
        //invalid previous booking
        $data = array(
            'is_valid' => 0
        );
        $this->db->set($data)->where('reservation_id',$this->session->userdata('old_reservation_id'))->update('ticket');
        //inserting new tickets into db
        ...
        //subtracting km from user tabel of previous booking
        $set_cond1 = "km - ".$this->session->userdata('old_km');
        $this->db->where('user_id', $this->session->userdata('user_id'));
        $this->db->set('km', $set_cond1, FALSE);
        $this->db->update('user');
        //adding km into user table table of current booking
    ...
    public function postbooking(){
        ...
        if($this->session->userdata('user_id')){
            ...
            $this->db->set('km', $set_cond, FALSE);
            $this->db->update('user');
            $new = $this->db->select('is_new,km,is_valued')->where('user_id',$this->session->userdata('user_id'))->get('user')->row();
            if($new->is_new == 1){
                $this->db->where('user_id', $this->session->userdata('user_id'));
                $this->db->set('is_new', '0', FALSE);
                $this->db->update('user');
            }
            else if($new->km >= 25000 && $new->is_valued == 0){
                $this->db->where('user_id', $this->session->userdata('user_id'));
                $this->db->set('is_valued', '1', FALSE);
                $this->db->update('user');
                }
            }
        else{
    ...
    public function get_ticket($t_id){
        $seatno = "";
        $ticket = $this->db->where('t_id',$t_id)->get('ticket')->row();
        $num_seat = $this->db->select('seatno')->where('reservation_id',$ticket->reservation_id)->get('ticket')->result_array();
        foreach($num_seat as $p){
            $seatno = $seatno.(string)$p['seatno'] . ',';
        }
        $num_seat = count($num_seat);
        if($ticket->trav_id){
            $user = $this->db->where('user_id',$ticket->trav_id)->get('user')->row();
            $a = array('user_id'         =>     $user->user_id);
            $this->session->set_userdata($a);
        ...
        $route = $this->db->where('route_id',$schedule->route_id)->get('route')->row();
        $tic_preview = array(
            'old_username'        =>     $user->fname.' '.$user->lname,
            'old_cnic'            =>     $user->cnic,
            'old_gender'          =>     $user->gender,
            'old_source'          =>     $route->source,
            'old_destination'     =>     $route->destination,
            'old_busdate'         =>     $ticket->trav_date,
            'old_departure'       =>     $route->departure,
            'old_arrival'         =>     $route->arrival,
            'old_day'             =>     $route->day,
            'old_state'           =>     $ticket->state,
            'old_reservation_id'  =>     $ticket->reservation_id,
            'old_t_id'            =>     $ticket->t_id,
            'old_offer_value'     =>     $ticket->offer_value,
            'old_offer_price'     =>     $ticket->offer_price,
            'old_km'              =>     $route->km,
            'old_fare'            =>     $route->fare,
            'old_seatno'          =>     $seatno,
            'old_num-seat'        =>     $num_seat
        );
        $this->session->set_userdata($tic_preview);
    }
    ...
    public function refund($id){
        ...
        $this->db->set('is_valid', 0)->where('t_id', $id)->update('ticket');
    }

    public function getListexcel($dates){
        return $this->db->where('created_at >=',$dates['sdate'])->where('created_at <=',$dates['ldate'])->get('ticket')->result_array();
    }
}
?>