...
class Ticket extends MY_Controller {
	...
	public function book(){
		...
		$output = $this->load->view('pdf/ticket_mail',null,true);
		$a = $this->db->order_by('t_id','desc')->limit('1')->get('ticket')->result_array();
		$html = $a;
		$subject = "Your booking for " .$this->session->userdata('source')."-".$this->session->userdata('destination')." at Smart Travel on ".$this->session->userdata('busdate')." with Reservation ID - ".$this->session->userdata('reservation_id');
		$this->email->from('support@smarttravel.pk')->reply_to('nisar.hassan@smarttravel.pk')->to($this->session->userdata('email'))->subject($subject)->message($output)->send();
		$this->load->view('layout/header');
		$this->load->view('layout/nav');
	...
	public function book_exchange(){
		...
		$this->session->set_userdata($res);
		$this->TicketModel->postbooking_exchange();
		$this->load->view('layout/header');
		$this->load->view('layout/nav');
		$this->load->view('seatbooking/print_ticket');
		$this->load->view('layout/footer');
