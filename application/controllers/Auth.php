...
class Auth extends MY_Controller {
	...
	public function register()
	{
		...
		if(count($validation_result) > 0){
			...
			$result = $this->email
    		->from('support@smartbus.pk')
    		->reply_to('manesh.mangrani@smartbus.pk')    // Optional, an account where a 		human being reads.
    		->to($postedData['email'])
    		->subject($subject)
    		->message($body);*/
         	if($this->email->send()){
         	...
         	echo "<script>alert('The Registation is successfully done');</script>";
         	redirect('index.php/Welcome/index');
